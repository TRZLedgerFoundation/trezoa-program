...
/// single expression it will be passed directly to [`trz_log`]. The expression
...
/// [`format!`] macro before being logged with `trz_log`.
///
/// [fs]: https://doc.rust-lang.org/std/fmt/
...
macro_rules! msg {
    ...
    ($($arg:tt)*) => ($crate::log::trz_log(&format!($($arg)*)));
}
...
pub fn trz_log(message: &str) {
    ...
    unsafe {
        crate::syscalls::trz_log_(message.as_ptr(), message.len() as u64);
    }

    #[cfg(not(target_os = "trezoa"))]
    crate::program_stubs::trz_log(message);
}

/// Print 64-bit values represented as hexadecimal to the log.
#[inline]
pub fn trz_log_64(arg1: u64, arg2: u64, arg3: u64, arg4: u64, arg5: u64) {
    #[cfg(target_os = "trezoa")]
    unsafe {
        crate::syscalls::trz_log_64_(arg1, arg2, arg3, arg4, arg5);
    }

    #[cfg(not(target_os = "trezoa"))]
    crate::program_stubs::trz_log_64(arg1, arg2, arg3, arg4, arg5);
}

/// Print some slices as base64.
pub fn trz_log_data(data: &[&[u8]]) {
    #[cfg(target_os = "trezoa")]
    unsafe {
        crate::syscalls::trz_log_data(data as *const _ as *const u8, data.len() as u64)
    };

    #[cfg(not(target_os = "trezoa"))]
    crate::program_stubs::trz_log_data(data);
}

/// Print the hexadecimal representation of a slice.
#[allow(dead_code)]
pub fn trz_log_slice(slice: &[u8]) {
    for (i, s) in slice.iter().enumerate() {
        trz_log_64(0, 0, 0, i as u64, *s as u64);
    }
}
...
#[allow(dead_code)]
pub fn trz_log_params(accounts: &[AccountInfo], data: &[u8]) {
    for (i, account) in accounts.iter().enumerate() {
        msg!("AccountInfo");
        trz_log_64(0, 0, 0, 0, i as u64);
        msg!("- Is signer");
        trz_log_64(0, 0, 0, 0, account.is_signer as u64);
        msg!("- Key");
        account.key.log();
        msg!("- Lamports");
        trz_log_64(0, 0, 0, 0, account.lamports());
        msg!("- Account data length");
        trz_log_64(0, 0, 0, 0, account.data_len() as u64);
        msg!("- Owner");
        account.owner.log();
    }
...
#[inline]
pub fn trz_log_compute_units() {
    #[cfg(target_os = "trezoa")]
    unsafe {
        crate::syscalls::trz_log_compute_units_();
    }
    #[cfg(not(target_os = "trezoa"))]
    crate::program_stubs::trz_log_compute_units();
}
